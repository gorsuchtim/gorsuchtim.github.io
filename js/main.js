...
function viewport__test() {
  if (window.innerWidth <= 768) {
    // if < 768 add mobile class to project container
  ...
  function windowResize() {
    if (window.innerWidth <= 768) {
      // when resizing from desktop to mobile
      $(".portfolio__wrap").addClass("mobileView");
    } else if (window.innerWidth > 768) {
...
function portfolioInit() {
  ...
  function toggle__backgroundView() {
    $(".portfolio__container--demoOverlay")
      .toggleClass("noOverlay")
      .parent()
      .toggleClass("removeFilter");
  ...
  function getNextBackgroundImage(current) {
    // es6 this into a better solution
    var next,
      backgroundImages = [
        "arthill.jpg",
        "bevo.jpg",
    ...
    if (current === backgroundImages[backgroundImages.length - 1]) {
      // if current is the last array item
      next = backgroundImages[0]; // set current = to 1st array item to "loop"
    } else {
      for (var i = 0; i < backgroundImages.length; i++) {
        // iterate the array
        if (backgroundImages[i] === current) {
          // if an array item matches current
  ...
  function setNewBackgroundImage(nextBackground) {
    $(".portfolio__container--demo").css(
      "background-image",
      "url(images/portfolioIcons/" + nextBackground + ""
    );
  }
  ...
  */
  ...
  /*------------------------
    ...
    ---------------------*/
  function sequenceFade(element, state) {
    // sequentially fade in/out portfolio cards in projects container
    $("" + element + "").each(function(i) {
      // sequentially fade in each portfolio grid card
      setTimeout(function() {
        if (state === "in") {
          $("" + element + "")
            .eq(i)
  ...
  switch (moveIn) {
    case "title":
      ...
      $(".portfolio__container--demo").removeClass(
    ...
    case "profile":
      ...
      $(".portfolio__container--demo")
        .removeClass("backgroundRight")
        .addClass("backgroundLeft"); // slide background image left
      if ($(".portfolio__wrap").hasClass("mobileView")) {
        // keep mainInfo off screen in mobile (save real estate)
        $(".mainInfo").fadeOut();
      } else {
        $(".mainInfo").fadeIn();
      }
    ...
    case "projects":
      ...
      $(".portfolio__container--demo")
        .removeClass("backgroundLeft")
        .addClass("backgroundRight"); // slide background image right
      if ($(".portfolio__wrap").hasClass("mobileView")) {
        // keep mainInfo off screen in mobile (save real estate)
        $(".mainInfo").fadeOut();
      } else {
    ...
    case "demo":
      $(".portfolio__container--demo")
        .removeClass("backgroundLeft backgroundRight")
        .css("filter", "grayscale(0%)"); // remove slide from background image, remove grayscale from container so demo colors show
      $(".mainInfo").fadeOut();
      sequenceFade(".portfolioCard", "out"); // fade out portfolio cards
      break;
  }
...
$(".controlTabs").click(function(e) {
  ...
  switch (clicked) {
    case "control__mobile":
      if (!$(".controlTabs").hasClass("showing")) {
        // if control tabs currently DONT have class showing
        sequenceFade(".controlTabs", "in"); // sequentially fade them in
        $(".control__mobile").css(
          "background-image",
          'url("images/portfolioIcons/close_mobile.svg")'
        );
      } else {
        // if control tabs DO have class showing
        sequenceFade(".controlTabs", "out"); // sequentially fade them out
        $(".control__mobile").css(
          "background-image",
          'url("images/portfolioIcons/menuIcon.svg")'
        );
      }
      break;
    case "control__home":
    ...
    case "control__profile":
      if ($(".portfolio__container--profile").hasClass("offScreenTop")) {
        // if profile container is not currently visible AND menu IS visible
        moveContent("profile"); // move profile container in to view
      }
      break;
    case "control__projects":
      if ($(".portfolio__wrap").hasClass("mobileView")) {
        // test if viewport is in mobile view
        $(".portfolioCard").each(function(i) {
          // iterate over each portfolio card
          if (
            $(".portfolioCard")
              .eq(i)
              .hasClass("flipped")
          ) {
            // determine if any card is flipped
            $(".portfolioCard").show(); // show all cards previously hidden on flip
            $(".portfolioCard")
              .eq(i)
              .removeClass("flipped fullCard"); // and remove flipped and fullCard class to restore grid view to normal
          }
        });
      }
      if ($(".portfolio__container--projects").hasClass("offScreenTop")) {
        // if projects container is not currently visible AND menu IS visible
        moveContent("projects"); // move projects container in to view
      }
      break;
    case "control__source":
      if ($(".portfolio__container--demoOverlay").children().length > 0) {
        // if a project has been loaded into the demoOverlay container
...
/*-----------------------------
 ...
 -----------------------------*/
$(".portfolio__button").click(function(e) {
  // clicking title screen portfolio button
  var clicked = e.currentTarget; // get the element clicked
  if ($(clicked).hasClass("portfolio__button--profile")) {
    // if clicked element was profile button
    moveContent("profile");
  } else if ($(clicked).hasClass("portfolio__button--projects")) {
    // if clicked element was projects button
...
function setupProfileContainer() {
  ...
  setTimeout(function() {
    // use timing  function to fade in each list item one-by-one over 250ms
...
$(".profileCategory").click(function(e) {
  // on clicking to a new category
...
function changeProfileCategory(category) {
  if (!$(category).hasClass("profileCategoryActive")) {
    // if the clicked category is NOT the current active category
...
function changeProfileContent(clicked) {
  ...
  switch (
    clicked // determine which category was clicked & show the appropriate list
  ) {
  ...
  $(".contentListItem").each(function(i) {
    // sequentially show all previously hidden content list items over 1ms
    setTimeout(function() {
      $(".contentListItem")
        .eq(i)
        .addClass("showing");
...
$(".portfolioCard").click(function() {
  // flip portfolio cards when clicked
  if ($(".portfolio__wrap").hasClass("mobileView")) {
    // test for mobile behavior
    $(".portfolioCard")
      .not($(this))
      .hide(); // hide all cards except the clicked card
  ...
  $(this)
    .find(".back")
    .toggleClass("ieBackface hidden__none")
    .find(".demoButton")
    .focus(); // Toggle special class to reveal "back of card" for IE
});
...
function loadDemo(project, stylesheet) {
  ...
  $(".portfolio__container--demoOverlay").load(
    "htmltemplates/" + projectName + ".htm"
  ); // load html file into --demo
  $("#dynamicCSS").attr("href", stylesheet); // set dynamicCSS via stylesheet value passed in demoButton click event

  $.ajax({
    // load in appropriate javascript file via ajax call
...
---------------------------*/
$(".codeChoice").click(function(e) {
  // behavior for code display code choice
...
$(".mobileCodeClose").click(function() {
  $(".portfolio__container--code")
    .toggle()
    .toggleClass("offScreenTop"); // toggle code container in/out of view
