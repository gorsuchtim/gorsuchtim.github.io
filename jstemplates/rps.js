$(document).ready(function () {
            ...
            var compChoice, userResponse, result, cm1, dynamicCategory, roundCount = 0,
                largestTendency,
                rockTrend = 0,
                paperTrend = 0,
                scissorsTrend = 0,
                lastrockCatChanged, lastpaperCatChanged, lastscissorsCatChanged,
                tendencyCats = {
                    rockCat: [{
            ...
            $('.welcomeCard__button').click(function () { // on clicking through introCard tutorial        
                        ...
                        case 2:
                            ...
                            $(this).css('color', 'rgba(80,80,80,0.8)';
                                ...
                                case 3: // on clicking Play game
                                    tutorialOn = true; sequenceFade('.userOption h3', 'highlightUserAnswer', 100, 'add'); closeWelcomeCard(); // fade out the welcome card                  
                                    break;
                            }
                    });
                ...
                $('.userOption').click(function (e) {
                    ...
                    if ($('.rps__selected').length === 0) {
                        $(this).children().addClass('rps__selected');
                        ...
                        if (roundCount < 2) { // if this is the first round or second round
                ...
                function changeTutorialCard(card) {
                    ...
                    switch (card) {
                        case 2:
                            ...
                            $('.tooltip__text').html('Looks like ' + round1Result + '.  Click Continue below to advance.');
                            $('.continueWalkthrough').addClass('highlightContinue pulse');
                            break;
                        case 3:
                            tutorialCard = 3;
                            $('.tooltip__text').html('Since no data is available, the program randomly chose ' + compChoice + '.  Click Continue to advance.');
                            break;
                        case 4:
                            tutorialCard = 4;
                        ...
                        case 5:
                            ...
                            $('.tooltip__text').html('Though perhaps random, your choice of ' + userChoice + ' is logged as a reaction.  Select Statistics to view the data.');
                            break;
                        case 6:
                            tutorialCard = 6;
                            $('.tooltip__text').html('Select Rock, Paper, or Scissors.');
                        ...
                        case 7:
                            tutorialCard = 7;
                            $('.continueWalkthrough').addClass('highlightContinue pulse');
                            $('.tooltip__text').html('Last round the program chose ' + cm1 + '.  It is storing your choice of ' + userChoice + ' against ' + cm1 + ' to predict your next move.');
                            break;
                        case 8:
                            tutorialCard = 8;
                        ...
                        case 9:
                            tutorialCard = 9;
                            $('.continueWalkthrough').addClass('highlightContinue pulse');
                            $('.tooltip__text').html('The program also watches for trends.  If your response to Paper has been Rock at least 3x in a row, Rock is set as a trend.');
                            break;
                        case 10:
                            tutorialCard = 10;
                            $('.tooltip__text').html('If the program finds a trend it ignores stored data and strategizes in line with the trend.  Select Rock, Paper, or Scissors.');
                            break;
                        case 11:
                            tutorialCard = 11;
                            $('.continueWalkthrough').addClass('highlightContinue pulse');
                            $('.tooltip__text').html('Note the program choice of ' + compChoice + ' in response to your choice of ' + userChoice + '.');
                            break;
                        case 12:
                            tutorialCard = 12;
                            $('.tooltip__text').html('Select Rock, Paper, or Scissors to play the final round of the walkthrough');
                            break;
                        case 13:
                            tutorialCard = 13;
                            ...
                            $('.continueWalkthrough').addClass('highlightContinue pulse');
                            $('.tooltip__text').html('The program chose ' + compChoice + '.  In the last round you saw ' + cm1 + ' and your next choice has usually been ' + historicalTendency + '.');
                            break;
                        case 14:
                            tutorialCard = 14;
                ...
                $('#displayStats').click(function () { // clicking display stats, fade in data container
                ...
                $('.closeDataContainer').click(function () { // close data container
                ...
                function setupNextRound() {
                    ...
                    $(rps__round).text(round); // set element content to round number
                    $('.option').children().removeClass('rps__selected win lose tie'); // reset border colors and remove selected
                }
                ...
                function randomComp() {
                    ...
                    $('#comp' + compChoice + '').children().addClass('rps__selected'); // dynamically set computer choice text based on compchoice
                    seeWhoWon();
                }
                ...
                function setTendencies() {
                    ...
                    $('.' + tendencyCats[dynamicCategory][objProp].name + '').text(tendencyCats[dynamicCategory][objProp].value);

                    checkTrend(tendencyCats[dynamicCategory][objProp].name);
                }

                function checkTrend(cat) { // make this more dynamic - see above    
                    var temp = [];
                    var trend;
                    switch (dynamicCategory) {
                        case 'rockCat':
                            if (cat === lastrockCatChanged) { // see if the current object (rr|rp|rs) value that was changed matches the one changed last round                    
                            ...
                            break;
                        case 'paperCat':
                            if (cat === lastpaperCatChanged) { // see if the current object (rr|rp|rs) value that was changed matches the one changed last round                    
                            ...
                            break;
                        case 'scissorsCat':
                            if (cat === lastscissorsCatChanged) { // see if the current object (rr|rp|rs) value that was changed matches the one changed last round                    
                    ...
                    for (var i = 0; i < temp.length; i++) { // iterate over them to see if any index value >= 3
                        if (temp[0] >= 3) { // if so, run predictiveTrend with that argument
                            trend = 'rock';
                        } else if (temp[1] >= 3) {
                            trend = 'paper';
                        } else if (temp[2] >= 3) {
                    ...
                    if (trend !== undefined) { // if trend has a value then it means a value was set because one category has 3+ trend
                        predictiveTrend(trend);
                    } else { // otherwise we are looking at historical data for our choice
                ...
                function predictiveTrend(predChoice) {
                    ...
                    if (predChoice === 'rock') { // if user tends to mostly select rock after seeing the computer's most recent round choice
                        compChoice = 'Paper'; // then computer shall thwart him with paper
                    } else if (predChoice === 'paper') { //if user tends to mostly select paper after seeing the computer's most recent round choice
                        compChoice = 'Scissors'; // computer shall sink his battleship with scissors
                    } else if (predChoice === 'scissors') { // if user.... oh you get the point.
                        compChoice = 'Rock';
                    }
                    $('#comp' + compChoice + '').children().addClass('rps__selected'); // dynamically set computer choice text based on compchoice
                    seeWhoWon();
                ...
                function iterateDynamicObject() { // iterate over the dynamic object to retrieve all value properties of our dynamic object        
                ...
                function getMaxValue(arr) {
                    arr.sort(function (a, b) { // then sort those values, keeping the 'name' and 'value' properties tied together
                ...
                function predictiveHistory(predChoice) {
                    ...
                    if (predChoice === 'r') { // if user tends to mostly select rock after seeing the computer's most recent round choice
                        compChoice = 'Paper'; // then computer shall thwart him with paper
                    } else if (predChoice === 'p') { //if user tends to mostly select paper after seeing the computer's most recent round choice
                        compChoice = 'Scissors'; // computer shall sink his battleship with scissors
                    } else if (predChoice === 's') { // if user.... oh you get the point.
                        compChoice = 'Rock';
                    }
                    $('#comp' + compChoice + '').children().addClass('rps__selected'); // dynamically set computer choice text based on compchoice
                    seeWhoWon();
                ...
                function seeWhoWon() { // pretty self-explanatory.  base result variable on comparison of userResponse and compChoice to see who won
                    switch (userResponse) {
                        case 'userRock':
                            if (compChoice === 'Rock') {
                                result = 'Tie';
